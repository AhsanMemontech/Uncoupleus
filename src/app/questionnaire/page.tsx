...
export default function QuestionnairePage() {
  ...
  const handleComplete = (answers: Record<string, any>) => {
    // Analyze answers to determine eligibility
    const analysis = analyzeEligibility(answers)
    setResults({ answers, ...analysis })
    setShowResults(true)
  }

  const analyzeEligibility = (answers: Record<string, any>) => {
    // Updated analysis for the simplified 3-question questionnaire
    const criteria = {
    ...
    // Check if residency is met (critical requirement)
    ...
    // Additional recommendations based on marriage duration
              ...
              <div className="text-center mb-2">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center space-y-2 sm:space-y-0 sm:space-x-4">
                  ...
                  <div className="text-gray-300 text-sm sm:text-base">
                    <div>Score: {results.score}/{results.totalCriteria}</div>
                  ...
                  {Object.entries(results.criteria).map(([key, met]) => (
                    <div key={key} className="flex items-center justify-between p-3 rounded-lg" style={{ backgroundColor: 'hsla(0, 0%, 100%, 0.05)' }}>
                      <span className="text-gray-300 capitalize text-sm sm:text-base">
                        {key === 'residency' ? 'NY State Residency' :
                         key === 'marriage_duration' ? 'Marriage Duration' :
                         key === 'child_agreement' ? 'Child Custody Agreement' : key.replace('_', ' ')}
                      </span>
